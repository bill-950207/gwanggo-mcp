...
import { z } from 'zod'
import { generate, getTask, listModels, me, pollTask, GwanggoError } from './api.js'

const text = (s: string) => ({ content: [{ type: 'text' as const, text: s }] })
const errText = (e: unknown) => {
  const err = e as GwanggoError
  return { content: [{ type: 'text' as const, text: `Error${err.status ? ` (${err.status})` : ''}: ${err.message}` }], isError: true }
}

/** snake_case 입력을 API의 camelCase 필드로 매핑 */
function toBody(model: string, prompt: string, opts: Record<string, unknown>): Record<string, unknown> {
...
export async function serve(): Promise<void> {
  ...
  server.tool(
    ...
    { type: z.enum(['image', 'video']).optional().describe('Filter by model type') },
    async ({ type }) => {
      try {
  ...
  server.tool(
    ...
    { id: z.string().describe('Generation id returned by generate_image/generate_video') },
    async ({ id }) => {
      try {
