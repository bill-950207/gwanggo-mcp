...
function arg(flags, name) {
    ...
    if (i >= 0 && flags[i + 1] && !flags[i + 1].startsWith('--'))
        return flags[i + 1];
...
async function cmdGenerate(rest) {
    const kind = (rest[0] === 'video' ? 'video' : 'image');
    ...
    const imageUrl = arg(rest, 'image-url');
    if (imageUrl)
        body.imageUrl = imageUrl;
    const aspect = arg(rest, 'aspect-ratio');
    if (aspect)
        body.aspectRatio = aspect;
    const resolution = arg(rest, 'resolution');
    if (resolution)
        body.resolution = resolution;
    const duration = arg(rest, 'duration');
    if (duration)
    ...
    if (task.status === 'COMPLETED')
        ...
        console.error(`실패: ${task.error || 'unknown'} (크레딧 자동 환불)`);
        process.exit(1);
    }
    else
...
async function main() {
    ...
    try {
        ...
        switch (cmd) {
            ...
            case 'task': {
                if (!rest[0]) {
                    console.error('사용법: gwanggo-mcp task <id>');
                    process.exit(1);
                }
...
API 키 발급: ${API_URL}/dashboard/api-keys`);
        ...
        if (err.status === 401 && cmd !== 'login' && !getKey())
            console.error('먼저 `gwanggo-mcp login` 을 실행하세요.');
        process.exit(1);
    }
}
main();
